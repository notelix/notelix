export function normalizeEmbeddedLanguage(language) {
...
export function normalizeEmbeddedTheme(theme) {
  if (typeof theme !== "string") return "auto";
  const normalized = theme.trim().toLowerCase();
  return ["auto", "light", "dark"].includes(normalized) ? normalized : "auto";
}

const embeddedConfig =
  typeof window === "undefined" ? undefined : window.NotelixEmbeddedConfig;

export const embeddedLanguage = normalizeEmbeddedLanguage(
  embeddedConfig?.language,
);
export const embeddedTheme = normalizeEmbeddedTheme(embeddedConfig?.theme);

export const embeddedCopy =
  embeddedLanguage === "zh-CN"
    ? {
        cancel: "取消",
